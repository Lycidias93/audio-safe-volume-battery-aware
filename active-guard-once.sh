#!/system/bin/sh
# Audio Safe Volume Disabler bounded active-session guard.
# Foreground/manual only. Not a daemon, no network, no wakelock.
set -eu
...
LOG="$MODDIR/active-guard-once.log"
...
apply_once() {
  ...
  log "ACTIVE_GUARD phase=$phase safe_media_volume_enabled=$(settings_get safe_media_volume_enabled) audio_safe_volume_state=$(settings_get audio_safe_volume_state) audio_safe_csd_next_warning=$(settings_get audio_safe_csd_next_warning) audio_safe_csd_current_value=$(settings_get audio_safe_csd_current_value) dose_records=$(settings_get audio_safe_csd_dose_records)"
}

passes="$(normalize_number "${1:-$ACTIVE_GUARD_PASSES}")"
sleep_s="$(normalize_number "${2:-$ACTIVE_GUARD_SLEEP_SECONDS}")"
max_passes="$(normalize_number "$ACTIVE_GUARD_MAX_PASSES")"
max_sleep="$(normalize_number "$ACTIVE_GUARD_MAX_SLEEP_SECONDS")"
[ "$passes" -gt 0 ] || passes=12
[ "$sleep_s" -gt 0 ] || sleep_s=15
[ "$max_passes" -gt 0 ] || max_passes=24
[ "$max_sleep" -gt 0 ] || max_sleep=60
[ "$passes" -le "$max_passes" ] || passes="$max_passes"
[ "$sleep_s" -le "$max_sleep" ] || sleep_s="$max_sleep"

log "START active-guard-once module=$MODID name=$MODNAME version=$VERSION versionCode=$VERSION_CODE passes=$passes sleep_seconds=$sleep_s"

echo "== ASVD active guard once =="
echo "module=$MODID"
echo "name=$MODNAME"
echo "version=$VERSION"
echo "versionCode=$VERSION_CODE"
echo "passes=$passes sleep_seconds=$sleep_s"
echo "mode=bounded foreground/manual, no daemon, no network, no wakelock"

i=1
while [ "$i" -le "$passes" ]; do
  echo
  echo "== guard pass $i/$passes =="
  apply_once "pass_$i"
...
log "DONE active-guard-once module=$MODID name=$MODNAME version=$VERSION versionCode=$VERSION_CODE"
write_asvd_state "PASS"
echo
echo "RESULT: ASVD_ACTIVE_GUARD_ONCE_DONE"
